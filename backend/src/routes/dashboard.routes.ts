...
const dashboardQuerySchema = z.object({
  ...
  to: z
    ...
    .regex(/^\d{4}-\d{2}-\d{2}$/)
    .optional(),
  /** Optional GEO filter. When omitted or empty, returns aggregate across all GEOs. */
...
router.get("/", async (req, res, next) => {
  try {
    ...
    // ── 1. Fetch all distinct GEOs that have any data for this app ──────────
    ...
    // ── 2. Fetch reports for the selected date range ──────────────────────────
    const reportWhere = {
      appId: app.id,
    ...
    // ── 4. Compute per-GEO breakdown (for predict block + potential future use)
    ...
    // ── 5. Build funnel, kpis, trend, table, latest ───────────────────────────

    if (geoFilter) {
      // ─── Specific GEO mode: behaves like current (single GEO) ──────────────
    ...
    // ─── Aggregate mode: sum across all GEOs ────────────────────────────────

    // Aggregate funnel: sum the latest report per GEO for cumulative totals
    ...
    // Build a synthetic "latest" record for KPI computation
    const latestByDate = reports.reduce((best, r) => (r.date > best.date ? r : best), reports[0]);
    // For revenue/adSpend we sum the latest-per-geo values
    let aggRevenueDay = 0;
    let aggAdSpend = 0;
    ...
    // Trend: group by date, sum across GEOs
